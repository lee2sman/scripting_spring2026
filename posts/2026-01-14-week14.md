...
* Compelling design. Not a copycat or LLM/chat-whatever job.
* Mobile responsive design (must work on desktop and phone. Design for phone should be considered, not random and broken!)
* Uses data fed from an external json file, which holds your station URLs, titles, colors, or any other needed data.
* Is capable of playing (and pausing and switching between) radio stations.&nbsp;
* Uses webaudio.

**Bonus (optional) goal**:

* Make use of localStorage to store some persistent data between sessions. Examples include: if a user clicks the star next to a station, it puts that station on the top of the list. Or maybe a user can select between site themes, choose or upload an avatar, or even add their own station stream URLs that stay persistent between sessions.
